...
file.write("id" + ',' + "Epoches stopped" + ',' + 'Accuracy' + ',' +'Time(second)' + '\n')
...
def load_data_keras(dataset,case):
    ...
    edges_unordered = np.genfromtxt("data/{}/{}_{}/input/{}_{}.cites".format(dataset,dataset,case,dataset,case), dtype=np.int32)
...
for jjj in id_file:
    print("Current Id is {}".format(jjj))
    t0 = time.time()
    ...
    df.to_csv('data/{}/{}_{}/output/performance_{}_output_{}_id_{}.csv'.format(FLAGS.dataset,FLAGS.dataset,FLAGS.case,FLAGS.dataset,FLAGS.case,jjj),index=True,sep=',')
    file = open('performance_{}_{}.csv'.format(FLAGS.dataset,FLAGS.case),'a')
    file.write(str(jjj) + ','  + str(epoch) + ',' + str(round(test_acc,4)*100) + ',' + str(round(time.time() - t0,4)) + '\n')
    file.close()
