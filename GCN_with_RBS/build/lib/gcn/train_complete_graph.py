...
flags.DEFINE_string('case', 'complete_graph', 'Case string.')  # 'org','random_graph','random_features','random_both','no_features','no_graph','complete_graph'
...
def load_data_keras(dataset,case):
    ...
    if FLAGS.dataset == "constructive":
        ...
        num_nodes = 1858
    adj = sp.csr_matrix(np.ones((num_nodes,num_nodes), dtype=np.int32) - np.eye(num_nodes, dtype=np.int32))
    edges = (num_nodes * num_nodes - num_nodes)/2

    # build symmetric adjacency matrix
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)

    print('Dataset has {} nodes, {} edges, {} features.'.format(adj.shape[0], edges, features.shape[1]))
