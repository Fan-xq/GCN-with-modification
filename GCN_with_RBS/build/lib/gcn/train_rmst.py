...
flags.DEFINE_string('case', 'random_graph', 'Case string.')  # 'org','random_graph','random_features','random_both','no_features','no_graph','complete_graph'
...
def load_data_keras(dataset,case,rmst_gamma):
    ...
    edges_unordered = np.genfromtxt("data/{}/{}_{}/input/{}_{}_gamma_{}_k_1.cites".format(dataset,dataset,case,dataset,case,rmst_gamma), dtype=np.int32)
...
# rmst_gamma_list = np.round(np.arange(0.01,0.11,0.01),2)
rmst_gamma_list = np.round(np.arange(0.001,0.011,0.001),3)
id_file = list(range(1,6,1))

for iii in rmst_gamma_list:
    for jjj in id_file:
        print("Current rmst_gamma is {}".format(iii),"Current Id is {}".format(jjj))
        t0 = time.time()
        # Load data using keras input
        features, adj, y = load_data_keras(FLAGS.dataset,FLAGS.case,iii)
        ...
        df.to_csv('data/{}/{}_{}/output/performance_{}_output_{}_gamma_{}_k_1_id_{}.csv'.format(FLAGS.dataset,FLAGS.dataset,FLAGS.case,FLAGS.dataset,FLAGS.case,iii,jjj),index=True,sep=',')
