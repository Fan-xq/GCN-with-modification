...
def load_data_keras(dataset,case,spielman_epsilon):
    ...
    edges_unordered = np.genfromtxt("data/{}/{}_{}/input/{}_{}_epsilon_{}.cites".format(dataset,dataset,case,dataset,case,spielman_epsilon), dtype=np.int32)
...
spielman_epsilon_list = np.round(np.arange(0.1,1.1,0.1),1)
id_file = list(range(1,6,1))

for iii in spielman_epsilon_list:
    for jjj in id_file:
        print("Current spielman_epsilon is {}".format(iii),"Current Id is {}".format(jjj))
        t0 = time.time()
