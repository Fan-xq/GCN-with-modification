...
def load_data_keras(dataset,case,spielman_epsilon):
    ...
    edges_unordered = np.genfromtxt("data/{}/{}_{}/input/{}_{}_epsilon_{:.2f}.cites".format(dataset,dataset,case,dataset,case,spielman_epsilon), dtype=np.int32)
...
spielman_epsilon_list = np.round(np.arange(0.05,1.05,0.05),2)
id_file = list(range(1,6,1))

for iii in spielman_epsilon_list:
    for jjj in id_file:
        print("Current spielman_epsilon is {}".format(iii),"Current Id is {:.2f}".format(jjj))
        t0 = time.time()
        ...
        df.to_csv('data/{}/{}_{}/output/performance_{}_output_{}_epsilon_{}_id_{}.csv'.format(FLAGS.dataset,FLAGS.dataset,FLAGS.case,FLAGS.dataset,FLAGS.case,iii,jjj),index=True,sep=',')
